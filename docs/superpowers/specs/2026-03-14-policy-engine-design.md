...
**Key design principle:** The `PolicyStore` is protected by an `Arc<RwLock<_>>`. Many requests hold read locks concurrently with zero contention. Policy pushes (rare) take a write lock, compile and validate the new policy atomically, swap the store, and release. No request ever sees a partially-updated policy.
...
| `forbidden_pattern` | `Name, Regex` | Named forbidden content patterns (compiled at load time) |
...
- Patterns in policy facts are compiled to regex at policy load time, not at eval time
...
5. Compile patterns → compiled `Regex` objects stored alongside their relation
...
| Condition | Response |
|---|---|
| `PolicyStore` uninitialized | `Deny + Audit(Critical)` |
...
facts: agent("agt-1", "analyst") + tool_call(...) + content_tag("pii", "high")
expect: Allow, effects contains Redact("response.content", "pii")

// forbidden pattern in arg → deny regardless of role
facts: agent("agt-1", "admin") + tool_call(...) + call_arg("q", "DROP TABLE users")
